...
$fname = $lname = $gender = $address = $weight = $nic = $phoneNumber = $email = $bloodgroup = $dob = $password = $confirmPassword = "";
$fnameErr = $lnameErr = $genderErr = $addressErr = $weightErr = $nicErr = $phoneNumberErr = $emailErr = $bloodgroupErr = $dobErr = $passwordErr = $confirmPasswordErr = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") 
{
  ...
  else 
  {
    ...
    if (empty($fnameErr) && empty($lnameErr) && empty($genderErr) && empty($nicErr) && empty($phoneNumberErr) && empty($emailErr) && empty($dobErr) && empty($addressErr) && empty($weightErr) && empty($bloodgroupErr) && empty($passwordErr) && empty($confirmPasswordErr)) 
    {
        ...
        if (empty($emailErr) && empty($nicErr) && empty($usernameErr)) 
        {
            ...
            $stmt = $conn->prepare("INSERT INTO users (fname, lname, gender, dob, nic, address, weight, bloodgroup, pnumber, email, password) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("sssssssssss", $fname, $lname, $gender, $dob, $nic, $address, $weight, $bloodgroup, $phoneNumber, $email, $password);
...
<head>
    ...
    <link rel="stylesheet" href="donor registration style.css">
</head>
<body>
...
<hr class="topic_line">
<h1 class="topic"> Donor Registration</h1>

<form method="post" action="donor registration.php">
...
                <p>NIC Number</p>
    ...
    <div class="split_right">    
        <div class="right centerd">
            ...
            <p>Blood Group</p>
            <input type="text" placeholder="A+,A-,B+,B-,AB+,AB-,O+,O-" name="bloodgroup">
