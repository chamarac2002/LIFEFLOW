...
$fname = $lname = $gender = $address = $weight = $nic = $phoneNumber = $email = $bloodgroup = $dob = $password = "";
$fnameErr = $lnameErr = $genderErr = $addressErr = $weightErr = $nicErr = $phoneNumberErr = $emailErr = $bloodgroupErr = $dobErr = $passwordErr = "";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($fnameErr) && empty($lnameErr) && empty($phoneNumberErr) && empty($nicErr) && empty($passwordErr)) {
        $updateQuery = "UPDATE users SET fname = ?, lname = ?, gender = ?, nic = ?, pnumber = ?, email = ?, address = ?, weight = ?, bloodgroup = ?, dob = ?, password = ? WHERE email = ?";
        $stmt = $conn->prepare($updateQuery);
        $stmt->bind_param("ssssssssssss", $fname, $lname, $gender, $nic, $phoneNumber, $email, $address, $weight, $bloodgroup, $dob, $password, $email);
        if ($stmt->execute()) {
...
<h1 class="dp"> Donor Profile Update </h1>
...
<div class="qs"> 
    <form method="post" action="">   
        ...
        <p>Blood Group :</p>
        <input type="text" class="bloodgroup" name="bloodgroup" value="<?php echo $bloodgroup; ?>" placeholder="Blood group">
